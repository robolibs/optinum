#include <doctest/doctest.h>
#include <optinum/lina/basic/matmul.hpp>
#include <optinum/lina/decompose/lu.hpp>

namespace lina = optinum::lina;
using optinum::simd::Matrix;
using optinum::simd::Vector;


TEST_CASE("lina::lu reconstructs P*A = L*U") {
    dp::mat::Matrix<double, 3, 3> a;
    a(0, 0) = 0.0;
    a(1, 0) = 1.0;
    a(2, 0) = 2.0;
    a(0, 1) = 2.0;
    a(1, 1) = 1.0;
    a(2, 1) = 0.0;
    a(0, 2) = 1.0;
    a(1, 2) = 0.0;
    a(2, 2) = 1.0;

    const auto f = lina::lu(Matrix<double, 3, 3>(a));
    CHECK(!f.singular);

    const auto P = lina::permutation_matrix<double, 3>(f.p);
    const auto lhs = lina::matmul(P, a);
    const auto rhs = lina::matmul(f.l, f.u);

    for (std::size_t i = 0; i < 3; ++i) {
        for (std::size_t j = 0; j < 3; ++j) {
            CHECK(lhs(i, j) == doctest::Approx(rhs(i, j)));
        }
    }
}

TEST_CASE("lina::lu_solve solves Ax=b") {
    dp::mat::Matrix<double, 2, 2> a;
    a(0, 0) = 4.0;
    a(1, 0) = 2.0;
    a(0, 1) = 1.0;
    a(1, 1) = 3.0;

    dp::mat::Vector<double, 2> b;
    b[0] = 1.0;
    b[1] = 2.0;

    const auto f = lina::lu(Matrix<double, 2, 2>(a));
    const auto x = lina::lu_solve(f, Vector<double, 2>(b));
    // Verify A*x == b - wrap x in view for matmul
    const auto bx = lina::matmul(Matrix<double, 2, 2>(a), Vector<double, 2>(x));
    CHECK(bx[0] == doctest::Approx(b[0]));
    CHECK(bx[1] == doctest::Approx(b[1]));
}
