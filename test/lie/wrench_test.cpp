...
TEST_CASE("Wrench construction from vectors") {
    simd::Vector<double, 3> force{1.0, 2.0, 3.0};
    simd::Vector<double, 3> torque{0.1, 0.2, 0.3};
...
TEST_CASE("Wrench construction from Vector6") {
    simd::Vector<double, 6> vec{1.0, 2.0, 3.0, 0.1, 0.2, 0.3};
    Wrenchd wrench(vec);
...
TEST_CASE("Wrench mutators") {
    ...
    SUBCASE("set_force from Vector3") {
        simd::Vector<double, 3> force{4.0, 5.0, 6.0};
    ...
    SUBCASE("set_torque from Vector3") {
        simd::Vector<double, 3> torque{0.4, 0.5, 0.6};
        wrench.set_torque(torque);
        CHECK(vec_approx_equal(wrench.torque(), torque));
    }

    SUBCASE("set_vector") {
        simd::Vector<double, 6> vec{7.0, 8.0, 9.0, 0.7, 0.8, 0.9};
...
TEST_CASE("Wrench frame transformation") {
    ...
    SUBCASE("pure rotation") {
        ...
        // Rotate 90° around z
        SE3d T(SO3d::rot_z(std::numbers::pi / 2), simd::Vector<double, 3>{0.0, 0.0, 0.0});
...
TEST_CASE("Wrench power computation") {
    ...
    SUBCASE("dot product with Vector6") {
        Wrenchd wrench(1.0, 2.0, 3.0, 0.1, 0.2, 0.3);
        simd::Vector<double, 6> v{0.5, 1.0, 1.5, 0.05, 0.1, 0.15};
...
TEST_CASE("Physical scenario: robot arm") {
    ...
    SE3d T_base_ee(SO3d::rot_z(std::numbers::pi / 4), simd::Vector<double, 3>{0.5, 0.0, 0.3});
