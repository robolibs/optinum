...
namespace optinum::simd {
    ...
    // Returns mask where true (all bits set) indicates infinity
    // =========================================================================
    template <typename T, std::size_t W> OPTINUM_INLINE mask<T, W> isinf(const pack<T, W> &x) noexcept {
        mask<T, W> result;
        for (std::size_t i = 0; i < W; ++i) {
            // Set all bits to 1 if inf, 0 otherwise
            T mask_val;
            if (std::isinf(x.data_[i])) {
