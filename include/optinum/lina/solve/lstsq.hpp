...
namespace optinum::lina {
    ...
    [[nodiscard]] dp::Result<simd::Vector<T, N>, dp::Error> try_lstsq(const simd::Matrix<T, M, N> &a,
        ...
        for (std::size_t ii = 0; ii < N; ++ii) {
            ...
            if (diag == T{}) {
                return dp::Result<simd::Vector<T, N>, dp::Error>::err(dp::Error::invalid_argument("rank deficient R"));
            }
            x[i] = sum / diag;
        }

        return dp::Result<simd::Vector<T, N>, dp::Error>::ok(x);
    }

    template <typename T, std::size_t M, std::size_t N>
    [[nodiscard]] simd::Vector<T, N> lstsq(const simd::Matrix<T, M, N> &a, const simd::Vector<T, M> &b) noexcept {
